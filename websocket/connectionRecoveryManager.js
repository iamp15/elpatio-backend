...
class ConnectionRecoveryManager {
  ...
  registerDisconnection(socket) {
    ...
    if (activeTransactions.length === 0) {
      console.log(`🧹 [RECOVERY] ${userType} ${userId} sin transacciones activas, limpiando inmediatamente`);
      this.cleanupImmediately(socket.id);
      return;
    }
    ...
    console.log(`⏳ [RECOVERY] ${userType} ${userId} desconectado con ${activeTransactions.length} transacciones activas. Tiempo de gracia: ${gracePeriod}ms`);
  ...
  async handleReconnection(socket, userId) {
    ...
    if (!disconnectionInfo) {
      console.log(`ℹ️ [RECOVERY] ${userType} ${userId} reconectó pero no hay desconexión pendiente`);
      return { recovered: false };
    }
    
    const disconnectionDuration = Date.now() - disconnectionInfo.timestamp;
    console.log(`✅ [RECOVERY] ${userType} ${userId} reconectado dentro del periodo de gracia (${disconnectionDuration}ms)`);
    ...
    this.notifySuccessfulReconnection(socket, recoveredTransactions, disconnectionDuration);
  ...
  async rejoinTransactionRoom(socket, transaccionId) {
    try {
      console.log(`🔄 [RECOVERY] Re-uniendo socket ${socket.id} a transacción ${transaccionId}`);
      ...
      if (!transaccion) {
        console.error(`❌ [RECOVERY] Transacción ${transaccionId} no encontrada`);
        return;
      }
      
      // Enviar estado actual al socket reconectado
      socket.emit('transaction-state-recovered', {
      ...
      console.log(`✅ [RECOVERY] Socket re-unido a transacción ${transaccionId} en estado: ${transaccion.estado}`);
    } catch (error) {
      console.error(`❌ [RECOVERY] Error re-uniendo a transacción ${transaccionId}:`, error);
    }
  }
  ...
  handleGracePeriodExpired(socketId) {
    ...
    console.log(`⏰ [RECOVERY] Periodo de gracia expirado para ${disconnectionInfo.tipo} ${disconnectionInfo.userId}`);
  ...
  async handleTransactionDisconnectionTimeout(transaccionId) {
    try {
      ...
      console.log(`⚠️ [RECOVERY] Transacción ${transaccionId} perdió conexión. Estado: ${transaccion.estado}`);
      
      // Según el estado, decidir qué hacer
      switch (transaccion.estado) {
        case 'pendiente':
          // Si estaba pendiente, se puede dejar pendiente (no se perdió nada)
          console.log(`ℹ️ [RECOVERY] Transacción ${transaccionId} estaba pendiente, se mantiene así`);
          break;
        ...
        case 'en_proceso':
          ...
          console.log(`⚠️ [RECOVERY] Transacción ${transaccionId} estaba en proceso, se mantiene para completar manualmente`);
          ...
          transaccion.metadata = {
            ...
            notasSistema: 'Conexión perdida durante procesamiento. Verificar estado manualmente.',
          };
        ...
        case 'realizada':
          ...
          console.log(`🔴 [RECOVERY] Transacción ${transaccionId} estaba REALIZADA - Usuario reportó pago, cajero debe verificar`);
          
          transaccion.metadata = {
            ...transaccion.metadata,
            desconexionCriticaDetectada: true,
            timestampDesconexion: new Date().toISOString(),
            notasSistema: 'IMPORTANTE: Usuario reportó pago pero se desconectó. Verificar pago urgente.',
          };
          await transaccion.save();
          break;
        ...
        case 'completada':
          ...
          console.log(`✅ [RECOVERY] Transacción ${transaccionId} ya estaba completada`);
          break;
          
        default:
          console.log(`ℹ️ [RECOVERY] Transacción ${transaccionId} en estado ${transaccion.estado}`);
      }
      ...
      console.error(`❌ [RECOVERY] Error manejando timeout de transacción ${transaccionId}:`, error);
    }
  }
  ...
  getActiveTransactions(socketId) {
    ...
    for (const [transaccionId, sockets] of this.socketManager.roomsManager.rooms.transacciones.entries()) {
   ...
   * Notificar reconexión exitosa
   */
  notifySuccessfulReconnection(socket, recoveredTransactions, disconnectionDuration) {
    ...
    recoveredTransactions.forEach(transaccionId => {
      this.socketManager.roomsManager.notificarTransaccion(
        ...
        'participant-reconnected',
        {
          ...
          userId: socket.userType === 'jugador' ? socket.telegramId : socket.cajeroId,
          transaccionId,
