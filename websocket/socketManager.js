...
class SocketManager {
  ...
  async authenticateJugador(socket, data) {
    ...
    try {
      ...
      console.log(`👤 Jugador autenticado: ${jugador.nickname || jugador.firstName} (${telegramId})`);
  ...
  handleSolicitarDeposito(socket, data) {
    if (socket.userType !== 'jugador') {
      socket.emit('error', { message: 'Solo jugadores pueden solicitar depósitos' });
      return;
    }

    console.log(`💰 Solicitud de depósito de jugador ${socket.telegramId}:`, data);
    ...
    socket.emit('solicitud-enviada', { message: 'Solicitud enviada a cajeros' });
  }
  ...
  handleAtenderDeposito(socket, data) {
    if (socket.userType !== 'cajero') {
      socket.emit('error', { message: 'Solo cajeros pueden atender depósitos' });
      return;
    }
  ...
  handleConfirmarDeposito(socket, data) {
    if (socket.userType !== 'cajero') {
      socket.emit('error', { message: 'Solo cajeros pueden confirmar depósitos' });
      return;
    }
  ...
  handleRechazarDeposito(socket, data) {
    if (socket.userType !== 'cajero') {
      socket.emit('error', { message: 'Solo cajeros pueden rechazar depósitos' });
      return;
    }
    ...
    console.log(`❌ Depósito rechazado por cajero ${socket.cajeroId}: ${motivo}`);
  }
}
